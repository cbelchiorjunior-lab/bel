...
// ===================================================
// CUSTOMIZER — CANAL YOUTUBE (IDs dos vídeos)
// ===================================================
add_action( 'customize_register', 'ct_customize_youtube' );
function ct_customize_youtube( $wp_customize ) {
    $wp_customize->add_section( 'ct_youtube', array(
        'title'    => 'Canal YouTube',
        'priority' => 50,
    ) );

    for ( $i = 1; $i <= 6; $i++ ) {
        $wp_customize->add_setting( "ct_youtube_video_{$i}", array(
            'default'           => '',
            'sanitize_callback' => 'sanitize_text_field',
        ) );
        $wp_customize->add_control( "ct_youtube_video_{$i}", array(
            'label'       => "ID do Vídeo {$i}",
            'description' => 'Cole apenas o ID do vídeo (ex: dQw4w9WgXcQ)',
            'section'     => 'ct_youtube',
            'type'        => 'text',
        ) );
    }
